...
from ..config import settings, MEXICAN_CONFIG, SPANISH_ERROR_RESPONSES
...
class KavakSalesAgent:
    ...
    def _build_chat_history(self, conversation_history: Optional[List[Dict]]) -> List[BaseMessage]:
    ...
    def _optimize_for_whatsapp(self, response: str) -> str:
        ...
        if len(response) > settings.response_max_length:
            response = response[:settings.response_max_length - 50] + "...\n\n¿Te interesa saber más detalles? 😊"
    ...
    def _add_contextual_emoji(self, response: str) -> str:
        ...
        elif any(word in response.lower() for word in ["precio", "pago", "financiamiento"]):
