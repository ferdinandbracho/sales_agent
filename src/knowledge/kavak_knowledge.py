...
class KavakKnowledgeBase:
    ...
    def _categorize_content(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['garantía', 'garantia', 'cobertura', 'protección']):
            return 'warranty'
        elif any(word in text_lower for word in ['financiamiento', 'credito', 'préstamo', 'pago']):
            return 'financing'
        elif any(word in text_lower for word in ['ubicación', 'sede', 'sucursal', 'dirección']):
            return 'locations'
        elif any(word in text_lower for word in ['proceso', 'compra', 'venta', 'como funciona']):
            return 'process'
    ...
    def search_knowledge(self, query: str, top_k: int = 3) -> List[str]:
        ...
        if not self.knowledge_chunks:
            return ["No tengo información específica sobre eso, pero en Kavak ofrecemos garantía, financiamiento y el mejor servicio."]
        ...
        return results if results else ["Información no encontrada en la base de conocimiento."]
    ...
    def get_kavak_info(self, query: str) -> str:
        ...
        try:
            ...
            if relevant_chunks and relevant_chunks[0] != "Información no encontrada en la base de conocimiento.":
