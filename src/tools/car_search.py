...
# Load car data
CAR_DATA_PATH = os.path.join(os.path.dirname(__file__), "../../data/sample_caso_ai_engineer.csv")
...
def load_car_data() -> pd.DataFrame:
    ...
    try:
        ...
        df['descripcion'] = df.apply(lambda row: 
...
"""
        ...
        for idx, auto in resultados.iterrows():
            bluetooth_text = "✅ Bluetooth" if auto['bluetooth'] == 'Sí' else "❌ Sin Bluetooth"
            carplay_text = " • ✅ CarPlay" if pd.notna(auto['car_play']) and auto['car_play'] == 'Sí' else ""
...
"""
        ...
        for idx, auto in auto_encontrado.iterrows():
            ...
            carplay_icon = "✅" if pd.notna(auto['car_play']) and auto['car_play'] == 'Sí' else "❌"
...
def buscar_con_fuzzy_matching(df: pd.DataFrame, marca: str, modelo: str) -> pd.DataFrame:
...
def obtener_autos_populares() -> str:
    ...
    try:
        ...
        respuesta = f"{MEXICAN_CONFIG['emojis']['car']} **Autos más populares en Kavak:**\n\n"
